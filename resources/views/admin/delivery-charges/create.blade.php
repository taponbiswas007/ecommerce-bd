...
@section('content')
    <form action="{{ route('admin.delivery-charges.store') }}" method="POST">@csrf
            ...
            function loadUpazilas(district, selected) {
                ...
                $.getJSON('{{ url('/delivery-charges/upazilas') }}', {
                    ...
                    .fail(function(jqxhr, status, error) {
                        ...
                        $('#upazila_list').empty().append(
                            '<div class="px-2 text-danger">' + msg + '</div>');
                        openDropdown(document.getElementById('upazila_display'));
                    });
            }

            // If there's an initial district (e.g., form repopulated), display it
            var initialDistrict = $('#district_input').val();
            if (initialDistrict) {
                $('#district_display').val(initialDistrict);
                loadUpazilas(initialDistrict, $('#upazila_input').val());
            }
        });
    </script>
@endpush
