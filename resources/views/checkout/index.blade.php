                            ...
                            <div class="form-group">
                                ...
                                <div class="select-wrapper">
                                    <select name="shipping_method" id="shipping_method"
                                        ...
                                        required>
                                        <option value="" {{ old('shipping_method') ? '' : 'selected' }}>-Select
                                ...
                                <div class="item-image">
                                    @php
                                        ...
                                        if ($item->product) {
                                            ...
                                            $selectedImage = $primaryImage ?: $firstImage;

                                            if ($selectedImage) {
                                                $img =
                                                    $selectedImage->image_url ??
                                                    asset('storage/' . $selectedImage->image_path);
                                            }
                                        }
                                    @endphp
    ...
    <script>
        (function() {
            function initShippingDropdowns() {
                ...
                var initialShippingAmount = parseAmount($('#shipping_amount_display').text());
                var initialTotalAmount = parseAmount($('.total-amount').first().text());
                var baseTotalWithoutShipping = initialTotalAmount - initialShippingAmount;

                function updateDisplayedTotal(shippingAmount) {
                    var totalAmount = baseTotalWithoutShipping + parseAmount(shippingAmount);
                    ...
                    $.getJSON('{{ route('shipping.estimate') }}', {
                        ...
                        .done(function(data) {
                            if (data && data.total !== undefined) {
                                var shippingAmount = parseFloat(data.total || 0);
                                $('#shipping_amount_display').text(shippingAmount.toFixed(2));
                                updateDisplayedTotal(shippingAmount);
